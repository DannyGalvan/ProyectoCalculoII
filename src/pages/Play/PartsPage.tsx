...
import { byPartsProblems } from '../../config/byParts';
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <Col md="8" className="mb-3 flex flex-col">
        <Card className="rounded-2xl border-0 bg-secondary shadow-[1px_10px_8px_7px_#B46767]">
        ...
        >
          {shuffleArray(problem?.steps[currentStep]?.options ?? []).map(
            (item, index) => (
              <Card
                key={index}
                className="max-w-[30%] cursor-pointer rounded-2xl border-0 bg-secondary shadow-[1px_10px_8px_7px_#B46767] hover:scale-105 md:max-w-[200px] lg:max-w-[62%]"
              >
                <img
                  key={index}
                  src={item.image}
      ...
      <Final open={final} setOpen={toggleFinal} url='/IntegralByParts' />
