...
import { bySubstitutionProblems } from '../../config/bySubstitution';
...
const SubstitutionPage = () => {
  ...
  const { currentStep, handleClick, message, problem, selectedItem, process } =
    usePlay({
      id,
      problems: bySubstitutionProblems,
      toggleError,
      toggleSuccess,
      toggleFinal,
      key: 'bySubstitution',
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <Col md="8" className="mb-3 flex flex-col">
        <Card className="rounded-2xl border-0 bg-welcome shadow-[1px_10px_8px_7px_#DC6620]">
        ...
        <Col xs="12" md="10" className="px-10 md:px-16">
          <Card
            className={`flex ${currentStep !== problem?.steps.length ? 'h-[45vh]' : 'h-[55vh]'
              } flex-col rounded-2xl border-0 bg-welcome py-2 shadow-[1px_10px_8px_7px_#DC6620] ${currentStep !== problem?.steps.length
                ? 'md:h-[55vh]'
                : 'md:h-[65vh]'
              }`}
          >
        ...
        >
          {shuffleArray(problem?.steps[currentStep]?.options ?? []).map(
            (item, index) => (
              <Card
                key={index}
                className="max-w-[30%] cursor-pointer rounded-2xl border-0 bg-welcome shadow-[1px_10px_8px_7px_#DC6620] hover:scale-105 md:max-w-[200px] lg:max-w-[62%]"
              >
                <img
                  key={index}
                  src={item.image}
      ...
      <Final open={final} setOpen={toggleFinal} url='/IntegralsBySubsitution' />
